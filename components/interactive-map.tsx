...
export function InteractiveMap({ layers }: MapProps) {
  ...
  const [groundStations, setGroundStations] = useState([
    { id: 1, name: "Delhi NCR", lat: 28.6139, lng: 77.209, pm25: 156, aqi: "Severe", trend: "up" },
    { id: 2, name: "Mumbai", lat: 19.076, lng: 72.8777, pm25: 89, aqi: "Moderate", trend: "stable" },
    { id: 3, name: "Bangalore", lat: 12.9716, lng: 77.5946, pm25: 67, aqi: "Satisfactory", trend: "down" },
    { id: 4, name: "Chennai", lat: 13.0827, lng: 80.2707, pm25: 78, aqi: "Moderate", trend: "stable" },
    { id: 5, name: "Kolkata", lat: 22.5726, lng: 88.3639, pm25: 134, aqi: "Very Poor", trend: "up" },
    { id: 6, name: "Hyderabad", lat: 17.385, lng: 78.4867, pm25: 92, aqi: "Moderate", trend: "down" },
  ])

  // Initialize Leaflet map
  useEffect(() => {
    if (!mapContainer.current || leafletMap) return
  ...
  useEffect(() => {
    if (leafletMap && mapLoaded && leafletLib) {
      addStationMarkers(leafletMap, leafletLib)
    }
  }, [groundStations, leafletMap, mapLoaded, leafletLib, layers.groundStations])
  ...
  const handleSearch = (query: string) => {
    const station = groundStations.find((s) => s.name.toLowerCase().includes(query.toLowerCase()))
    ...
    <div className="relative h-full w-full rounded-lg overflow-hidden bg-slate-50 border border-slate-200">
      ...
      <div className="absolute top-3 right-3 flex gap-2 z-10">
        ...
        <ExportDialog type="map" data={exportMapData()}>
          <Button variant="outline" size="sm" className="bg-white/95 backdrop-blur-sm border-slate-200">
      ...
      <div className="absolute bottom-3 left-3 z-10">
        <div className="bg-white/95 backdrop-blur-sm rounded-lg shadow-sm border border-slate-200 p-3">
          <h4 className="font-medium text-xs mb-2 text-slate-700">PM2.5 (µg/m³)</h4>
          ...
          <div className="text-center p-8">
            ...
            <div className="grid grid-cols-1 gap-2 max-w-md">
              {groundStations.slice(0, 4).map((station) => (
                <div key={station.id} className="bg-white p-3 rounded-lg shadow flex justify-between items-center">
                  <div>
                    <div className="font-medium text-sm">{station.name}</div>
                    <div className="text-xs text-slate-600">{station.aqi}</div>
                  </div>
                  <div className="text-right">
                    <div className="font-bold" style={{ color: getMarkerColor(station.pm25) }}>
                      {station.pm25}
                    </div>
