...
interface EnhancedForecastPoint {
  timestamp: number
  pm25: number
  pm10: number
  co: number
...
export class EnhancedPollutionPredictor {
  async generateComprehensiveForecast(lat: number, lon: number, days = 7): Promise<EnhancedForecastPoint[]> {
    try {
      // Get air quality forecast
      const airQualityData = await openMeteoClient.getAirQuality(lat, lon, { days })

      // Get weather forecast
      const weatherData = await openMeteoClient.getWeatherForecast(lat, lon, { days })
      ...
      const hoursToPredict = Math.min(days * 24, airQualityData.hourly.time.length)
      ...
      for (let i = 0; i < hoursToPredict; i++) {
        ...
        const forecastPoint: EnhancedForecastPoint = {
          ...
          so2,
          o3,
          aqi: openMeteoClient.calculateAQI(pm25),
          confidence: this.calculateConfidence(i, days),
          weatherFactors: {
            temperature: weatherData.hourly.temperature_2m[weatherIndex] || 20,
            humidity: weatherData.hourly.relative_humidity_2m[weatherIndex] || 50,
            windSpeed: weatherData.hourly.wind_speed_10m[weatherIndex] || 5,
            windDirection: weatherData.hourly.wind_direction_10m[weatherIndex] || 0,
          ...
          healthRecommendations: this.generateHealthRecommendations(pm25, pm10, o3, no2),
        }
  ...
  async getAirQualityTrends(lat: number, lon: number, days = 30): Promise<any> {
    try {
      ...
      const currentAirQuality = await openMeteoClient.getAirQuality(lat, lon, { days: 1 })
  ...
  private analyzeTrends(historical: any, current: any): any {
    ...
    const recentTemp = historical.hourly.temperature_2m.slice(-24).reduce((a: number, b: number) => a + b, 0) / 24
    const recentHumidity =
      historical.hourly.relative_humidity_2m.slice(-24).reduce((a: number, b: number) => a + b, 0) / 24
    const recentWind = historical.hourly.wind_speed_10m.slice(-24).reduce((a: number, b: number) => a + b, 0) / 24
    ...
    return {
      ...
      humidity: {
        ...
        impact: recentHumidity > 80 ? "pollution_accumulation" : "normal_dispersion",
      },
      wind: {
        recent: recentWind,
  ...
  private generateHealthRecommendations(pm25: number, pm10: number, o3: number, no2: number): string[] {
  ...
  private generateMockForecast(hours: number): EnhancedForecastPoint[] {
    ...
    for (let i = 0; i < hours; i++) {
      ...
      forecast.push({
        ...
        healthRecommendations: this.generateHealthRecommendations(pm25, pm25 * 1.5, 100, 50),
      })
    }
